...
Fast local gate — mirrors the `.husky/pre-commit` hook and remains the core of CI:
...
- Web build: `npm run build` (wasm-pack `--target web` emits raw output in `pkg/`, then `scripts/assemble-dist.mjs` builds a clean `dist/` from `static/` + the generated JS/WASM, `scripts/cache-bust.mjs` stamps the JS/CSS/WASM/service-worker URLs with `?v=<git-sha>`, and `scripts/verify-build.mjs` checks the deploy artifact).
...
- Local run: `npm run dev` (builds, then serves `dist/` on port 8000). Needs a WebGPU-capable browser.
- Build hardening: `npm run check:js` syntax-checks repo-owned JavaScript, `npm run audit` runs npm + RustSec advisory checks, `npm run smoke` browser-smokes the built `dist/` artifact, and `npm run smoke:live` checks the deployed site (accepting a graceful WebGPU-unsupported page on machines without a usable adapter).
...
- Audio (Web Audio): `src/audio.rs` (the synthesized layered node graph as a context-agnostic `Graph` — a hard-panned detuned drone pad, an octave-below sub-bass sine, a high twinkling starfield (per-voice LFOs), per-note oscillators, a camera-orbit stereo panner over the pad + starfield, three sends — a procedurally-generated convolver reverb with early reflections, a feedback delay, and a 4×-oversampled octave-up shimmer (a `2x²−1` waveshaper into the reverb) — plus a live subsonic high-pass and compressor. The `AudioEngine` drives it live (look-ahead scheduler on the AudioContext clock; owned by `AppState` as an `Option`, built lazily on first enable so the context starts inside the user gesture); `render_offline` drives the *same* `Graph` on an `OfflineAudioContext` from a composed-piece arrangement timeline (the headless `generate_piece`). No sample files).
- Arrangement (pure): `src/arrangement.rs` (`Arrangement` — a deterministic `seed`+`duration` cinematic A→B→C arc that drives BOTH the visuals (camera pose + normalised live physics, applied in `lib.rs`) and the audio (a synthetic `GalaxyState` timeline rendered offline), so a composed piece's picture and sound stay locked. No web/audio deps; native-tested).
- Mastering (pure): `src/mastering.rs` (offline mastering + analysis DSP on planar stereo `f32` — stereo-balance correction, subsonic high-pass, mono-bass sum, ITU-R BS.1770 integrated-loudness measurement + normalisation to a target LUFS, a look-ahead true-peak limiter to a -1 dBTP ceiling, raised-cosine fades, 24-bit WAV encoding, and a `MasterReport`. No web/audio deps, so it unit-tests natively).
...
- Frontend: `static/index.html` (WebGPU support check, loading/error UI, WASM bootstrap, control wiring: scenario dropdown + a restart icon, body-count / speed (reads Myr/s) / disk-temp (Toomre Q) / gas-fraction / bulge-fraction / gravity / star-size / volume sliders, mute button, a grouped **Dark matter halo** section (model dropdown + strength slider (reads km/s) + a Size slider for the scale radius (`set_halo_concentration`, reads kpc) + a Show toggle that drives `set_halo_visible`, and a Curve toggle for the rotation-curve overlay — a `<canvas>` chart fed by the `rotation_curve` / `elapsed_myr` exports, redrawn on gravity/halo/size/bulge changes), a `setupInfoButtons` pass that adds a "?" by each control label (descriptions in one map, a shared popover), a Sentry feedback button (hidden until `sentry.js` attaches the form — `static/sentry.js` loads the feedback-capable SDK bundle and calls `attachTo` only when a DSN is configured), and a draggable cog toggle pinned top-left (the chrome is on the body, so the cog never shifts when the panel opens/closes; position is not persisted) that also closes on an outside click or a scenario switch; a `?compose=SEED&dur=SECONDS[&particles=N]` URL param auto-plays the arrangement (once `is_ready()`) and exposes a headless `window.galacto` API (`isReady` / `whenReady` / `startArrangement` / `setParticleCount` / `fps` / `renderPieceTo`) so the automated pipeline can drive it without the UI — the setters silently no-op until the async WebGPU init finishes, so callers must await readiness; the soundscape auto-starts on the visitor's first interaction; registers the service worker), `static/styles.css`.
- Production scripts: `scripts/produce.mjs` (one command — `npm run produce -- --seed N --duration S` — builds, serves the verified `dist/` artifact, and runs the capture in `--produce` mode → a finished captioned MP4; defaults to a 10-min piece at 4K and `--particles 32768`); `scripts/capture-canvas-video.mjs` (headless-Chrome canvas capture; `--compose <seed>` plays the arrangement, `--particles N` sets the body count, `--produce` also renders the matching audio via `window.galacto.renderPieceTo`, muxes, and adds captions — it samples `window.galacto.fps()` during the record and warns if the rate dropped below target); `scripts/add-video-captions.mjs` (SVG→PNG start/end caption overlays via `rsvg-convert`, HEVC/MP4 out). The three share `scripts/cli.mjs` (arg parsing + a `run` helper); `capture` also takes `--reuse-chunks` to resume from an existing chunk dir without re-recording.
