...
- The soundscape is driven only by CPU-side visual state (camera, scenario, the live sim knobs) assembled into a `GalaxyState` each frame — it never reads GPU body data back, so the no-readback rule holds. Keep the generative logic (`music.rs`) free of web/audio dependencies so it stays native-testable; keep all Web Audio (`web-sys`) in `audio.rs`. All sound is synthesized — no sample/wave files or external audio sources.
