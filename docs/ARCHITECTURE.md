...
The scenario dropdown, the body-count slider, the Restart button, and the disk-temperature slider all funnel through `Simulation::reseed(queue, Reseed { scenario, temp, count, gravity, halo_v0, halo_kind })`, which regenerates the bodies and re-uploads the particle buffer, adopts the new active `count`, *and* rewrites the `SimulationParams` uniform (scenarios use different softening). It restarts the galaxy from fresh initial conditions — switch scenarios freely, change the resolution, or sweep the spiral disk clumpy → spiral → smooth.
