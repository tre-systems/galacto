...
**Tuned for calm and awe.** The design follows the relaxation literature: a slow, sparse note grid kept in the ~50–85 BPM range (near a resting heart rate, never frantic), soft note onsets (long fade-ins, no startle-inducing transients), consonant scales with no repeating melody, and a gentle **~0.1 Hz amplitude swell on the bed** (`breathing()` in `audio.rs`) — the cardiovascular resonance frequency (≈6 breaths/min) that maximises HRV, acting as a passive breathing pacer. The sense of awe comes from *vastness held calm*: the deep sub-bass, the long cavernous reverb, and the high shimmer give scale, while dynamics expand and contract slowly (the breath, the core swells, layers opening as you zoom) rather than through abrupt crescendos.
