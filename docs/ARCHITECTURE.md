...
2. **`update_audio()`** — assemble a `GalaxyState` snapshot from the camera (zoom/rotation), the live sim knobs, and the galaxy's own core dynamics (central mass + radial flux from the latest GPU readback), then let the `AudioEngine` glide the drone + effects toward it and schedule any due notes (a no-op until sound is enabled). See [Audio](#audio).
...
- **`music.rs` (pure, native-tested).** A `MusicEngine` turns a per-frame `GalaxyState` — assembled by `AppState` from the camera (zoom + rotation speed), the active `Scenario`, the live knobs, and, as the *primary* drivers, the galaxy's own core dynamics from the GPU readback (`core_mass` = how much mass has gathered at the centre; `core_flux` = matter moving in/out of it; `core_activity` = core churn) — into a `DroneTarget` (the sustained pad's voice pitches, brightness, level, and detune) and a stream of `NoteEvent`s (sparse, scale-quantised "starlight" notes). It maps each scenario to a musical character (a serene major/Lydian for the lone disks; darker, busier Dorian/Phrygian/Aeolian for the collisions), wanders a random walk over that scale for the melody, and scales note density, register, and loudness with the core's churn and gathered mass. It carries no web/audio dependency, so it unit-tests natively like `scenarios.rs`.
...
**How the visuals drive it.** The galaxy's core dynamics are the centre of it: **central mass** swells the pad's body and lifts note density; **radial flux** makes the pad breathe — matter collapsing inward raises it into tension and brightens it, matter streaming back out lets it settle; **core churn** opens the reverb and drives note density and shimmer. The camera and knobs layer on top — **zoom** sets the master brightness and reverb mix (close = bright and dry; far = dark and cavernous), **rotation speed** stirs the delay and detune, **scenario** swaps the scale and pad chord, **gravity** adds a slow pitch lean. The pad sits an octave below the scenario root — a deep bed — and its filter resonance and the reverb depth drift on slow, free-running LFOs, so the space keeps breathing even when the galaxy is momentarily still.
