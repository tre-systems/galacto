...
- **`Scenario::Spiral`** (`generate_disk`) — a heavy central bulge body (`BULGE_MASS`) plus an **exponential disk** of lighter stars (`STAR_MASS`), radii sampled so surface density ∝ e^(−r/`DISK_RD`), thin in `z`. The disk's summed mass dominates its own region (a "maximal disk"), making it spiral-prone. Each star gets a **prograde circular velocity** (bulge + enclosed disk + the active halo profile, via `circular_velocity`) plus a **random velocity dispersion** set per-radius from the **Toomre stability parameter Q** (`toomre_sigma`: σ_R = Q · 3.36 G Σ(r) / κ(r), with the `TOOMRE_SOFT` softening/thickness correction). Q ≲ 1 fragments into clumps, Q ≫ 2 is a featureless smear, and **spiral arms** (swing amplification) live in the Q ≈ 1–2 sweet spot. A fraction of the disk is tagged as dissipative **gas** — cooled toward circular, in-plane orbits each step (in `kick_drift_half`) so it stays cold, gathers into the arms, and renders blue. The bulge's share of the mass is set by the bulge fraction (`bulge_mass_for`). Softening is small (`SPIRAL_SOFTENING = 12`) so self-gravity stays sharp.
