...
3. **`render()`** — open a command encoder, then:
   ...
   - record the **core-statistics reduction** for the audio (`reduce_core`, throttled — skipped while a prior readback is still mapped): a workgroup tree-reduction of windowed central mass + radial flux into a 64-entry partials buffer, copied to a mappable staging buffer (mapped back asynchronously after submit);
...
| Reductions buffer | `64 × vec4<f32>` core-statistics partials (one per workgroup), for the audio | `STORAGE \| COPY_SRC` |
| Reduction staging | mappable copy of the reductions buffer (1 KB), read back asynchronously | `COPY_DST \| MAP_READ` |
