(() => {
  ...
  const loadSentry = () => {
    ...
    script.onload = () => {
      ...
      if (typeof window.Sentry.feedbackIntegration === "function") {
        integrations.push(
          window.Sentry.feedbackIntegration({
            ...
            messagePlaceholder:
              "What's working, what's broken, or what you'd love to see?",
          }),
        );
      }
      ...
      const feedback =
        typeof window.Sentry.getFeedback === "function"
          ? window.Sentry.getFeedback()
          : null;
      if (feedback) {
        const attach = () => {
  ...
  if (navigator.onLine === false) {
    window.addEventListener("online", loadSentry, { once: true });
    return;
  }
